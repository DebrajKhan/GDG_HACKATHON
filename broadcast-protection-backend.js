...
const steg      = require('./steg-engine');
...
class HLSSessionServer {
  constructor(sessionManager, forensicWatermarker, options = {}) {
    ...
    this.stegPipelines       = new Map(); // sessionId -> SteganographyPipeline
  }
  ...
  /**
   * Fetch, Watermark (Stealth), and Encrypt a segment for a specific session.
   */
  processSegment(session, profile, seq) {
    // 1. Get or Create the Stealth Steg Pipeline for this session
    if (!this.stegPipelines.has(session.id)) {
        this.stegPipelines.set(session.id, new steg.SteganographyPipeline(session));
    }
    const pipeline = this.stegPipelines.get(session.id);

    // 2. Ask watermarker which variant (A or B) to send
    this.forensicWatermarker.gopIndex = seq;
    const selection = this.forensicWatermarker.selectSegment(session);
    
    // 3. Load the pre-encoded file
    let rawSegment;
    try {
        rawSegment = fs.readFileSync(selection.segmentPath);
    } catch (err) {
        rawSegment = Buffer.alloc(188 * 1000, 0x47);
    }

    // 4. --- STEALTH DNA INJECTION (Layer B) ---
    // In production, we process the raw frames. 
    // Here we use the logic from steg-engine.js to sign the segment.
    const securedSegment = pipeline.processFrame(rawSegment, 1920, 1080);

    // 5. Encrypt for this specific session
    const keyInfo = this.sessionManager.getCurrentKey(session.id);
...
function createRoutes(sessionManager, hlsServer, leakDetector) {
  ...
  router.get('/hls/:sessionId/:profile/seg_:seq.ts', (req, res) => {
    ...
    try {
      ...
      const encryptedSeg = hlsServer.processSegment(session, profile, parseInt(seq));
...
function decodeViewerToken(token, req) {
  ...
  try {
    // Verifies the token against your secret key
    ...
    throw new AuthError('Invalid or expired viewer token');
  }
}
...
function loadSegmentFromVault(profile, seq) {
  /**
   * PRODUCTION LOGIC:
   * 1. Check local cache/vault for pre-encoded segment
   * 2. If not found, fetch from S3/CDN
   * 3. Return Buffer
   */
  const vaultDir = path.join(process.cwd(), 'vault', profile);
  const segmentPath = path.join(vaultDir, `seg_${seq}.ts`);

  if (fs.existsSync(segmentPath)) {
    return fs.readFileSync(segmentPath);
  }

  // Demo Fallback: Return a simulated TS packet if vault is empty
  // In a real environment, this would throw an error or fetch from CDN
  return Buffer.alloc(188 * 1000, 0x47); // Simulated TS Sync byte
}
